...
import clip
import ds 
from ds import prepare_coco_dataloaders, prepare_flickr_dataloaders, prepare_cub_dataloaders, prepare_flo_dataloaders, prepare_scannet_dataloaders
...
def get_features_uncertainty(BayesCap_Net, xfI, xfT):
    #recons
    outs = multi_fwpass_ProbVLM(BayesCap_Net, xfI, xfT)
    ...
    for j in range(xfI.shape[0]):
        r_dict['i_f'].append(xfI[j,:])
        r_dict['t_f'].append(xfT[j,:])
        r_dict['ir_f'].append(outs[0][0][j,:])
        r_dict['tr_f'].append(outs[1][0][j,:])
        u = get_GGuncer(outs[0][1][j,:], outs[0][2][j,:], c1=3, c2=2.8)
        r_dict['i_au'].append(u)
...
def geometric_mean(tensor):
    ...
    tensor = tensor[~torch.isinf(tensor)]
