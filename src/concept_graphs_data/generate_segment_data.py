...
def main(cfg : DictConfig):
    ...
    # save path
    segment_fp = scannet_data_root / 'finetune_data' / 'segments' / split 
    ...
    for scene_name in tqdm(scenes):
        ...
        for idx in trange(len(dataset)):
            ...
            for obj in fg_detection_list:
                ...
                scene_objects['object_pcd'].append(obj['pcd'])
        ...
        ######################## SELECT ALL THOSE SEGMENTS THAT ARE SIMILAR TO THE CORE CLASSES ##########################
        
        # get the sim between the core classes and the objects
        object_feats = torch.stack(scene_objects['object_feats'])
        ...
        # store a set of the ids of all relevant objects
        relevant_objects = set()
        # get the top-5 objects for each class and add them to an overall list
        for i in range(len(core_feats)):
            # get the top 5 objects
            top5_sim, top5_idx = torch.topk(sim[i], TOP_K_SCENE)

            # append to the relevant objects
            for idx in top5_idx:
                relevant_objects.add(idx.item())

        ################### CAPTION THE RELEVANT SEGMENTS AND ADD THEM TO AN OVERALL LIST OF OBJECTS ########################
        ...
        # caption the objects
        for idx in tqdm(relevant_objects):
            # get the path and bbox
            color_path = scene_objects['image_path'][idx]
            feature = scene_objects['object_feats'][idx]
            scene_name = scene_objects['scene_name'][idx]
            x_min, y_min, x_max, y_max = scene_objects['bbox'][idx][0]

            # get a caption for the object
            prompt = "USER: <image>\nDescribe the appearance of the central object in the image in one sentance, using the format 'an image of a <object description'. ASSISTANT:"
            image_object = Image.open(color_path)
            ...
            # crop the image
            cropped_image = image_object.crop((x_min, y_min, x_max, y_max))

            try:
                inputs = processor(images=cropped_image, text=prompt, return_tensors="pt").to("cuda:0")
            ...
            # add object to scene pcd
            # segmented_objects = {'feature': [], 'caption': [], 'scene_name': [], 'core_classes': []}
            segmented_objects['feature'].append(feature)
            segmented_objects['scene_name'].append(scene_name)
            segmented_objects['caption'].append(output)
            segmented_objects['core_classes'].append([])
            segmented_objects['image_path'].append(color_path)
            segmented_objects['bbox'].append(scene_objects['bbox'][idx])
        ...
        ################### USE LLM TO APPLY CORE CONCEPTS TO DESCRIPTIONS ########################

        del processor
        del model

        # get the sim between the core classes and the objects
        object_feats = torch.stack(segmented_objects['feature'])
        ...
        for i in tqdm(range(len(core_feats))):

            topk_sim, topk_idx = torch.topk(sim[i], TOP_K_SCENE)

            for j in range(len(topk_idx)):

                # get the caption of the selected object
                caption = segmented_objects['caption'][topk_idx[j]]
                # print the caption
                ...
                The field ’same_object’ should be set to 'True' if the captions could possibly refer to the same object, and 'False' if not. \
                ...
                if response:
                    segmented_objects['core_classes'][topk_idx[j]].append(train_classes[i])

        # # save the object 
        # print(f'size of segmented objects: {len(segmented_objects["feature"])}')
        # print(segmented_objects['caption'])   
        # print(segmented_objects['core_classes'])
        # print(segmented_objects['image_path'])
        # dt = h5py.special_dtype(vlen=str)
        # with h5py.File(segment_fp / f'{scene_name}.h5', 'w') as f:
        #     f.create_dataset('feature', data=segmented_objects['feature'])
        #     f.create_dataset('scene_name', data=segmented_objects['scene_name'], dtype=dt)
        #     f.create_dataset('caption', data=segmented_objects['caption'], dtype=dt)
        #     f.create_dataset('core_classes', data=segmented_objects['core_classes'], dtype=dt)
        #     f.create_dataset('image_path', data=segmented_objects['image_path'], dtype=dt)
        #     f.create_dataset('bbox', data=segmented_objects['bbox'])

        # save the object as pkl file
        with open(segment_fp / f'{scene_name}.pkl', 'wb') as f:
            pickle.dump(segmented_objects, f)

        # reload and print the number of objects
        with open(segment_fp / f'{scene_name}.pkl', 'rb') as f:
